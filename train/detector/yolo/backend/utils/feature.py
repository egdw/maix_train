...
class MobileNetFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size, weights=None, strip_size=32):
        import tensorflow as tf
        from tensorflow.keras.models import Model
        from tensorflow.keras.layers import Input
        from yolo.backend.utils.mobilenet_sipeed.mobilenet import MobileNet

        if type(weights) != str or not os.path.exists(weights):
...
class Inception3Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        inception = InceptionV3(input_shape=(input_size[0], input_size[1],3), include_top=False)
        if weights:
...
class VGG16Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        vgg16 = VGG16(input_shape=(input_size[0], input_size[1], 3), include_top=False)
        if weights:
...
class ResNet50Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        resnet50 = ResNet50(input_shape=(input_size[0], input_size[1], 3), include_top=False)
        if weights:
